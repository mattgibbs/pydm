...
class PyDMLabel(QLabel):
    ...
    # doesn't know what a color map is.  The following two color maps
    # are provided for that scenario.
    ...
    alarm_style_sheet_map = {
      ...
      ALARM_TEXT | ALARM_BORDER: {
        ALARM_NONE: "PyDMLabel {color: black; border-width: 2px; \
                     border-style: hidden;}",
        ...
        ALARM_DISCONNECTED: "PyDMLabel {color: white; border: \
                             2px solid white;}"
        }
      }
    ...
    def receivePrec(self, new_prec):
        if self._prec_from_pv:
            self.setPrecision(new_prec)
    ...
    def alarmSeverityChanged(self, new_alarm_severity):
        ...
        self.setStyleSheet(self.alarm_style_sheet_map[self._alarm_flags][
                                                        new_alarm_severity])
